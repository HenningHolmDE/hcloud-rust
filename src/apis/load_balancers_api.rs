...
pub struct ChangeReverseDnsEntryForThisLoadBalancerParams {
    ...
    pub change_reverse_dns_entry_for_this_load_balancer_request: Option<crate::models::ChangeReverseDnsEntryForThisLoadBalancerRequest>
}
...
pub async fn change_reverse_dns_entry_for_this_load_balancer(configuration: &configuration::Configuration, params: ChangeReverseDnsEntryForThisLoadBalancerParams) -> Result<crate::models::ChangeReverseDnsEntryForThisLoadBalancerResponse, Error<ChangeReverseDnsEntryForThisLoadBalancerError>> {
    let local_var_configuration = configuration;

    // unbox the parameters
    let id = params.id;
    let change_reverse_dns_entry_for_this_load_balancer_request = params.change_reverse_dns_entry_for_this_load_balancer_request;


    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/load_balancers/{id}/actions/change_dns_ptr", local_var_configuration.base_path, id=id);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());
    ...
    local_var_req_builder = local_var_req_builder.json(&change_reverse_dns_entry_for_this_load_balancer_request);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ChangeReverseDnsEntryForThisLoadBalancerError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
