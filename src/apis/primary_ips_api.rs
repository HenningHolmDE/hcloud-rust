...
/// Assign a Primary IP to a resource.  A Server can only have one Primary IP of type `ipv4` and one of type `ipv6` assigned. If you need more IPs use Floating IPs.  A Server must be powered off (status `off`) in order for this operation to succeed.  #### Operation specific errors  | Status | Code | Description | | --- | --- | --- | | `422` | `server_not_stopped` | The Server is running, but needs to be powered off | | `422` | `primary_ip_already_assigned` | Primary IP is already assigned to a different Server | | `422` | `server_has_ipv4` | The Server already has an IPv4 address | | `422` | `server_has_ipv6` | The Server already has an IPv6 address |
...
/// Unassign a Primary IP from a resource.  A Server must be powered off (status `off`) in order for this operation to succeed.  A Server requires at least one network interface (public or private) to be powered on.  #### Operation specific errors  | Status | Code | Description | | --- | --- | --- | | `422` | `server_not_stopped` | The Server is running, but needs to be powered off | | `422` | `server_is_load_balancer_target` | The Server IPv4 address is a loadbalancer target |
